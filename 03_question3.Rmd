...
Assuming $E[Y] = \hat p = 0.5$, such that $logit(\hat p) = X\beta = 0$, we can discard the predictor $x_1$, which is correlated with outcomes $Y$. If we only use the other two predictors, the model should converge.
...
data_3_0 <- data.frame(y, x2, x3) %>%
  ...
  mutate(x1 = case_when(y == 1 ~ abs(rnorm(1, mean = 0, sd = 0.5)),
                        y == 0 ~ -abs(rnorm(1, mean = 0, sd = 0.5))))
...
save(data_3_0, data_3_1, glm_con, glm_unc, 
...
(e). Perform a simulation study using the generative model in (a). 
...
For the Firth correction, you will get warning messages of the form "fitted probabilities numerically 0 or 1 occurred for variable V2"; 
you can disregard those. 

In the event that any of your simulation values do not converge, 
you may remove those from the simulation study.

Finally, for the Firth correction, 
you will want to set the maximum number of iterations to be at least 10000).
